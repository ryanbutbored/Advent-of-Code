#this is inefficient as hell but its only 1000 cards who cares
def sortCards(card : list, cards : list):
    scale = ["J", "2", "3", "4", "5", "6", "7", "8", "9", "T", "Q", "K", "A"] 
    score = 0
    for i, c in enumerate(card[0]):
        score += (scale.index(c)+1)*100**(4-i)
    card.append(score)
    cards.append(card)
    scores = []
    if len(cards) > 0:
    ...
    if "J" in cardDict.keys():
         match len(cardDict):
            ...
            case 2:
                index = 6
            case 3:
                if 3 in cardDict.values() or cardDict["J"] == 2:
                    index = 5
                else:
                    index = 4
            case 4:
